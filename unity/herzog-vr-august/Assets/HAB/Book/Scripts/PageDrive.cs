...
namespace Valve.VR.InteractionSystem
{
    ...
    public class PageDrive : BookComponent
    {
        public enum Axis_t
        {
            XAxis,
        ...
        // ebaender - stuff fix bad hover sphere transform on first attach
        ...
        private float currentCoverLimit;
        ...
        protected override void Start()
        {
            ...
            if (Book.pageMapping == null)
            {
                Book.pageMapping = GetComponent<LinearMapping>();
            }

            if (Book.pageMapping == null)
            {
                Book.pageMapping = gameObject.AddComponent<LinearMapping>();
            }
            ...
            if (limited)
            {
                ...
                if (forceStart)
                {
                    outAngle = Mathf.Clamp(startAngle, Book.hingeMinLimit, currentCoverLimit);
                }
            }
            else
            {
        ...
        private void UpdateLinearMapping()
        {
            if (limited)
            {
                ...
                lastMappingValue = Book.pageMapping.value;
                Book.pageMapping.value = (outAngle - Book.hingeMinLimit) / (currentCoverLimit - Book.hingeMinLimit);
            }
            else
            {
                // Normalize to [0, 1] based on 360 degree windings
                float flTmp = outAngle / 360.0f;
                Book.pageMapping.value = flTmp - Mathf.Floor(flTmp);
            }
        ...
        {
            if (!driving)
                transform.localRotation = Book.pagePhysicsController.transform.localRotation;
        }
            ...
            if (!toHandProjected.Equals(lastHandProjected))
            {
                ...
                if (absAngleDelta > 0.0f)
                {
                    ...
                    else
                    {
                        ...
                        if (limited)
                        {
                            float angleTmp = Mathf.Clamp(outAngle + signedAngleDelta, Book.hingeMinLimit, currentCoverLimit);

                            if (outAngle == Book.hingeMinLimit)
                            {
                                if (angleTmp > Book.hingeMinLimit && absAngleDelta < minMaxAngularThreshold)
                                {
                                    outAngle = angleTmp;
                                    lastHandProjected = toHandProjected;
                                }
                            }
                            else if (outAngle == currentCoverLimit)
                            {
                                if (angleTmp < currentCoverLimit && absAngleDelta < minMaxAngularThreshold)
                                {
                                    outAngle = angleTmp;
                                    lastHandProjected = toHandProjected;
                                }
                            }
                            else if (angleTmp == Book.hingeMinLimit)
                            {
                                outAngle = angleTmp;
                                lastHandProjected = toHandProjected;
                                onMinAngle.Invoke();
                            ...
                            else if (angleTmp == currentCoverLimit)
                            {
                                outAngle = angleTmp;
                                lastHandProjected = toHandProjected;
                                onMaxAngle.Invoke();
