...
namespace Valve.VR
{
    ...
    public class SteamVR_Skeleton_PoserEditor : Editor
    {
        ...
        // ebaender
        private SerializedProperty coverMappingProperty;
        private SerializedProperty pageMappingProperty;
        ...
        // ebaender - book handle parent object
        private Component bookHandleController;
        ...
        protected void OnEnable()
        {
            ...
            // ebaender
            coverMappingProperty = serializedObject.FindProperty("coverMapping");
            pageMappingProperty = serializedObject.FindProperty("pageMapping");
        ...
        protected void DrawPoseControlButtons()
        {
            ...
            // ebaender
            GUILayout.Space(32);
            GUILayout.Label("Cover Mapping:");
            coverMappingProperty.objectReferenceValue = EditorGUILayout.ObjectField(coverMappingProperty.objectReferenceValue, typeof(InteractionSystem.LinearMapping), true);

            GUILayout.Space(8);
            GUILayout.Label("Page Mapping:");
            pageMappingProperty.objectReferenceValue = EditorGUILayout.ObjectField(pageMappingProperty.objectReferenceValue, typeof(InteractionSystem.LinearMapping), true);
        ...
        void DrawPoseEditorMenu()
        {
            ...
            else
            {
                ...
                if (poseEditorExpanded.boolValue)
                {
                    ...
                    // ebaender
                    int poseSelected = GUILayout.SelectionGrid(activePoseIndex, poser.poseNames, 4);
        ...
        void DrawBlendingBehaviourMenu()
        {
            ...
            if (blendEditorExpanded.boolValue)
            {
                ...
                // ebaender - get handle controller for this poser
                bookHandleController = (Component)EditorGUILayout.ObjectField(bookHandleController, typeof(Component), true);

                // ebaender - auto-generate blending behaviours
                if (GUILayout.Button("*", GUILayout.MaxWidth(32)))
                {
                    if (bookHandleController != null)
                    {
                        // Collider[] colliders = bookHandleController.GetComponentsInChildren<Collider>();
                        for (int i = 0, j = 0; i < skeletonAdditionalPosesProperty.arraySize + 1; i++)
                        {
                            string blendName = poser.poseNames[i];
                            if (i == 0)
                            {
                                blendName = blendName.Split(' ')[0];
                            }
                            int blendCount = blendingBehaviourArray.arraySize;
                            blendingBehaviourArray.InsertArrayElementAtIndex(blendCount);
                            blendingBehaviourArray.GetArrayElementAtIndex(blendCount).FindPropertyRelative("name").stringValue = blendName;
                            blendingBehaviourArray.GetArrayElementAtIndex(blendCount).FindPropertyRelative("enabled").boolValue = true;
                            blendingBehaviourArray.GetArrayElementAtIndex(blendCount).FindPropertyRelative("influence").floatValue = 1;
                            blendingBehaviourArray.GetArrayElementAtIndex(blendCount).FindPropertyRelative("previewEnabled").boolValue = true;
                            blendingBehaviourArray.GetArrayElementAtIndex(blendCount).FindPropertyRelative("pose").intValue = j++;
                            serializedObject.ApplyModifiedProperties();
                        }
                        for (int k = 0; k < blendingBehaviourArray.arraySize; k++)
                        {
                            poser.blendingBehaviours[k].mask.Reset();
                        }
                    }
                    else
                    {
                        Debug.LogError("could not get book handle controller.");
                    }
                }
