...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Hand : MonoBehaviour
    {
        ...
        // ebaender - added these as public checkboxes for checked bools
        ...
        // ebaender - made these private and checked by debug controller
        private bool showDebugTextChecked;
        private bool spewDebugTextChecked;
        private bool showDebugInteractablesChecked;

        // ebaender - new debug collider visualizer for hover sphere
        private bool showDebugColliderChecked;

        // ebaender - console
        private string consoleOutput = null;
        private int logCounter = 0;

        // ebaender - ignore physical colliders
        public bool triggerCollidersOnly = false;

        // ebaender - no extra attachments
        public bool singleObjectOnly = false;
        ...
        // ebaender
        private GameObject colliderSphere = null;

        // ebaender
        public Collider LastCollider
        {
            get { return lastCollider; }
            set { lastCollider = value; }
        }
        ...
        {
            // ebaender - stop more than one object from attaching
            if (singleObjectOnly && attachedObjects.Count > 0)
            {
                return;
            }
        ...
        // ebaender - set checked debug flag values
        void CheckDebugFlags()
        {
            DebugController instance = InteractionSystem.DebugController.instance;
            bool debugActive = instance ? instance.active : true;
            showDebugTextChecked = showDebugText && debugActive;
            spewDebugTextChecked = spewDebugText && debugActive;
            showDebugInteractablesChecked = showDebugInteractables && debugActive;
            showDebugColliderChecked = showDebugCollider && debugActive;
        }

        //-------------------------------------------------
        protected virtual IEnumerator Start()
        {
            // ebaender - initialize debug collider sphere
            colliderSphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
            colliderSphere.GetComponent<Collider>().enabled = false;
            colliderSphere.GetComponent<MeshRenderer>().material = new Material(Shader.Find("Valve/VR/Silhouette"));
            colliderSphere.transform.parent = hoverSphereTransform;
            colliderSphere.transform.localPosition = new Vector3(0f, 0f, 0f);
        ...
        protected virtual bool CheckHoveringForTransform(Vector3 hoverPosition, float hoverRadius, ref float closestDistance, ref Interactable closestInteractable, Color debugColor)
        {
            ...
            for (int colliderIndex = 0; colliderIndex < overlappingColliders.Length; colliderIndex++)
            {
                ...
                // ebaender
                if (triggerCollidersOnly && !collider.isTrigger)
                    continue;
                ...
                // ebaender
                if (attachedObjects.Count == 0 && contacting == closestInteractable)
                {
                    // Debug.Log("HAND: " + collider + " " + closestInteractable);
                    lastCollider = collider;
                    lastInteractable = closestInteractable;
                }
            }

            if (showDebugInteractablesChecked && foundCloser)
            {
        ...
        {
            // ebaender - added debug controller check
            if (showDebugTextChecked)
            {
                if (debugText == null)
                {
                ...
                // ebaender - added collider debug line
                debugText.text = string.Format(
                    ...
                    handType.ToString(),
                    (lastCollider ? lastCollider.name : "null"),
                    logCounter,
        ...
        {
            // ebaender - updated checked debug flags
            CheckDebugFlags();

            // ebaender - update debug collider sphere
            colliderSphere.SetActive(showDebugColliderChecked);
            if (colliderSphere.activeSelf)
            {
                float diameter = hoverSphereRadius * 2f;
                colliderSphere.transform.localScale = new Vector3(diameter, diameter, diameter);
                if (hoveringInteractable)
                {
                    colliderSphere.GetComponent<MeshRenderer>().material.SetColor("g_vOutlineColor", new Color(1f, 1f, 0f));
                }
                else
                {
                    colliderSphere.GetComponent<MeshRenderer>().material.SetColor("g_vOutlineColor", new Color(1f, 1f, 1f));
                }
            }
